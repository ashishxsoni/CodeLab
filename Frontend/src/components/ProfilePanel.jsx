...
const ProfilePanel = ({ panelOpen, onClose }) => {
  ...
  const handleLogout = () => {
    ...
    axios
      .post(`${String(import.meta.env.VITE_API_URL)}/user/logout` , {} , { withCredentials: true }) //axios.post(url, data, config)
      .then((response) => {
    ...
    <div className="fixed inset-0 z-50 flex justify-end  bg-opacity-50 backdrop-blur-sm">
      ...
      >
        ...
        <div className={`flex flex-col items-center py-6 space-y-2 border-b ${darkMode ? "border-gray-900" : "border-gray-200"}`}>
          <motion.img
            src={user?.profileImage ? `${String(import.meta.env.VITE_API_URL)}${user.profileImage}` : "/images/man.png"}
            alt="User Profile"
