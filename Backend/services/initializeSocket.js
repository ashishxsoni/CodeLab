...
const initializeSocket = (server) => {
  const io = new Server(server, {
    cors: {
      origin: process.env.CLIENT_URL, // Explicitly list allowed origin
      methods: ["GET", "POST"],
    },
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("language-change", ({ roomId, language }) => {
      ...
      socket.to(roomId).emit("language-update", language , currentUser);
    });
    ...
    socket.on("leave-room", () => {
      ...
      if (!globalRooms.has(currentRoom)) {
        console.log(
          `Leave room event received but room ${currentRoom} doesn't exist in global state`
        );
        return;
      }
      ...
      if (currentUser.isHost) {
        ...
        socket.to(currentRoom).emit("room-closed");
        globalRooms.delete(currentRoom); 
      } else {
